...
def plot_enbr_distribution(data_dir, site_indices=None, nbr_norm=False, ref_data=None, ref_nbrs=None):
    ...
    for index, site_i in enumerate(enbr.keys()):
        ...
        if p_x_ref is not None:
            plt.plot(x, p_x_ref, antialiased=True, linewidth=1.0, color="green", label="Reference")
...
def plot_rtheta_distribution(data_dir, site_indices=None):
    ...
    if site_indices is None:
        ...
        rtheta_files = [f for f in os.listdir(data_dir) if f.endswith("r_theta.txt") and int(f[0:3]) in site_indices]
...
def main():
    ...
    plot_rtheta_distribution("test_1_angular_structure_data", site_indices=[1, 3])
